...
<ul style="font-size: 14px; list-style: disc; margin-left: 25px;">
	<li style="line-height: 21px; margin-bottom: 15px;">A plugin such as <a target="_blank" href="https://wordpress.org/plugins/wp-fastest-cache/">WP Fastest Cache</a> that creates static HTML files (which are lighter thanks to <?php echo WPACU_PLUGIN_TITLE; ?>) and reads them avoiding PHP code processing within the active plugins &amp; theme, including database queries which can use lots of server resources if you have high traffic. <?php echo sprintf(esc_html__('The page caching improves the %sTTFB%s (time to first byte) which measures the duration from the user or client making an HTTP request to the first byte of the page being received by the client\'s browser.', 'wp-asset-clean-up'), '<a href="https://wordpress.org/plugins/wp-fastest-cache/">', '</a>'); ?></li>
