...
class Maintenance
{
	...
	public static function updateAppendOrNotInlineCodeToCombinedAssets($isDebug = false)
	{
		...
		if ( ! Misc::isWpVersionAtLeast('5.5') ) {
			...
			if ($isDebug) {
				echo 'Версия WordPress ниже 5.5, поэтому встроенный код CSS/JS не добавляется к комбинированному списку CSS/JS.';
			}
		} else {
	...
	public static function clearCacheConditionally($isDebug = false)
	{
		...
		if ($wpacuLastClearCache && (strtotime( '-1 days' ) > $wpacuLastClearCache)) {
			...
			if ($isDebug) {
				echo 'Кэш был только что очищен, так как он не очищался в течение последних 24 часов.<br />';
			}
		}
	}
