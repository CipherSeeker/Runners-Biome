...
class Tools
{
	...
	public function getSystemInfo()
    {
	    ...
	    $return .= "\n" . '# PHP Extensions' . "\n";
	    $return .= 'cURL:                     ' . ( function_exists( 'curl_init' ) ? 'Поддерживается' : 'Не поддерживается' ) . "\n";
	    $return .= 'fsockopen:                ' . ( function_exists( 'fsockopen' ) ? 'Поддерживается' : 'Не поддерживается' ) . "\n";
	    $return .= 'SOAP Client:              ' . ( class_exists( 'SoapClient' ) ? 'Установлен' : 'Не установлен' ) . "\n";
	    $return .= 'Suhosin:                  ' . ( extension_loaded( 'suhosin' ) ? 'Установлен' : 'Не установлен' ) . "\n";
	    ...
	    $return .= 'Прочитал текст "Снятие жира":   '. (($settings['wiki_read'] == 1) ? 'Yes' : 'No') . "\n\n";

	    $return .= 'Управление в личном кабинете:             '. (($settings['dashboard_show'] == 1) ? 'Yes ('.$settings['dom_get_type'].')' : 'No');

	    if ( ! (isset($settings['show_assets_meta_box']) && $settings['show_assets_meta_box']) ) {
		    $return .= ' - Метабокс активов скрыт';
	    }

	    if ( isset($settings['hide_options_meta_box']) && $settings['hide_options_meta_box'] ) {
		    $return .= ' - Метабокс боковых опций скрыт';
	    }

	    $return .= "\n";

	    $return .= 'Управление во фронтенде:             '. (($settings['frontend_show'] == 1) ? 'Yes' : 'No') . "\n";

	    if ($settings['frontend_show'] == 1 && $settings['frontend_show_exceptions']) {
		    $return .= 'Не показывать интерфейсные ресурсы, если URI содержит (значение textarea):' . "\n" . $settings['frontend_show_exceptions'] . "\n\n";
	    }

	    $return .= 'Стиль полей ввода:                  '. ucfirst($settings['input_style'])."\n";
	    $return .= 'Скрыть файлы WP (от управления):       '. (($settings['hide_core_files'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Включить «Тестовый режим»?                  '. (($settings['test_mode'] == 1) ? 'Yes' : 'No') . "\n\n";

	    $return .= 'Минифицировать загруженный CSS?                   '. (($settings['minify_loaded_css'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Минифицировать загруженный JS?                    '. (($settings['minify_loaded_js'] == 1) ? 'Yes' : 'No') . "\n";

	    $return .= 'Объединить загруженный CSS?                  '. (($settings['combine_loaded_css'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Объединить загруженный JS?                   '. (($settings['combine_loaded_js'] == 1) ? 'Yes' : 'No') . "\n";
	    ...
	    $return .= 'Отключить эмодзи (для всего сайта)?                       '. (($settings['disable_emojis'] == 1) ? 'Yes' : 'No') . "\n";
        $return .= 'Отключить oEmbed (встраивание) (для всего сайта)?              '. (($settings['disable_oembed'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Отключить Dashicons, если панель инструментов (верхняя панель администратора) не отображается (для всего сайта)?        '. (($settings['disable_dashicons_for_guests'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Отключить редактор блоков CSS Gutenberg (для всего сайта)?   '. (($settings['disable_wp_block_library'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Отключить миграцию jQuery (для всего сайта)?               '. (($settings['disable_jquery_migrate'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Отключить ответ на комментарий (для всего сайта)?                '. (($settings['disable_comment_reply'] == 1) ? 'Yes' : 'No') . "\n\n";

	    $return .= 'Удалить тег ссылки «Really Simple Discovery (RSD)»?  '. (($settings['remove_rsd_link'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Удалить тег ссылки «Windows Live Writer»?            '. (($settings['remove_wlw_link'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Удалить тег ссылки "REST API"?                       '. (($settings['remove_rest_api_link'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Удалить тег «Короткая ссылка» для страниц/сообщений?               '. (($settings['remove_shortlink'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Удалить тег "Реляционные ссылки публикации"?             '. (($settings['remove_posts_rel_links'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Удалить метатег «Версия WordPress»?              '. (($settings['remove_wp_version'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Удалить все метатеги «генератор»?                '. (($settings['remove_generator_tag'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Удалить основную ссылку на RSS-канал?                        '. (($settings['remove_main_feed_link'] == 1) ? 'Yes' : 'No') . "\n";
	    $return .= 'Удалить ссылку на RSS-канал комментариев?                 '. (($settings['remove_comment_feed_link'] == 1) ? 'Yes' : 'No') . "\n";

	    $xmlProtocolStatus = 'Включено (по умолчанию)';
	    ...
	    if (isset($storageStats['total_size'], $storageStats['total_files'])) {
		    $return .= 'Всего кешированных файлов: '.$storageStats['total_files'].' ('.$storageStats['total_size'].') of which '.$storageStats['total_files_assets'].' are CSS/JS assets ('.$storageStats['total_size_assets'].')';
	    } else {
...
SQL;
	    ...
	    $return .= "\n" . '### Завершить информацию о системе ###';
	...
	public function doReset()
	{
		...
		if ($wpacuConfirmedValue !== 'yes') {
			exit('Ошибка: действие необходимо подтвердить.');
		}
...
SQL;
	...
	public function importDone()
    {
	    ...
	    foreach ($this->data['import_done_list'] as $importedKey) {
            ...
            // [wpacu_pro]
		    if ($importedKey === 'post_type_via_tax_exceptions') {
			    $importedMessage .= '<li>'.esc_html__('Load exceptions for all pages having specific taxonomies set', 'wp-asset-clean-up').'</li>';
		    }

            if ($importedKey === 'global_data') {
                $globalDataValues = $this->data['import_done_list']['global_data_values'];

                if ( (isset($globalDataValues['styles']['positions'])  && ! empty($globalDataValues['styles']['positions']))
                  || (isset($globalDataValues['scripts']['positions'])  && ! empty($globalDataValues['scripts']['positions'])) ) {
	                $importedMessage .= '<li>' . esc_html__( 'Any CSS/JS updated positions (to &lt;HEAD&gt; or &lt;BODY&gt;)', 'wp-asset-clean-up-pro' ) . '</li>';
                }

	            if ( (isset($globalDataValues['styles']['preloads'])  && ! empty($globalDataValues['styles']['preloads']))
                  || (isset($globalDataValues['scripts']['preloads'])  && ! empty($globalDataValues['scripts']['preloads'])) ) {
		            $importedMessage .= '<li>' . esc_html__( 'Any CSS/JS preloading (rel="preload")', 'wp-asset-clean-up-pro' ) . '</li>';
	            }

	            if ( isset($globalDataValues['scripts']['everywhere'])  && ! empty($globalDataValues['scripts']['everywhere']) ) {
                    $serialisedVal = maybe_serialize($globalDataValues['scripts']['everywhere']);

                    if (is_string($serialisedVal) && ( (strpos($serialisedVal, 'async') !== false) || (strpos($serialisedVal, 'defer') !== false) ) ) {
	                    $importedMessage .= '<li>' . esc_html__( 'Any defer/async attributes added site-wide to the JS files', 'wp-asset-clean-up-pro' ) . '</li>';
                    }
	            }

                if (    (isset($globalDataValues['plugins']) &&      ! empty($globalDataValues['plugins']))
                     || (isset($globalDataValues['plugins_dash']) && ! empty($globalDataValues['plugins_dash'])) )  {
                    $importedMessage .= '<li>' . esc_html__('Any plugin rules (e.g. unload, load exceptions) set in "PLUGINS MANAGER"') . '</li>';
                }
            }

            if ($importedKey === 'terms_metas') {
	            $importedMessage .= '<li>'.esc_html__('Terms\' Rules including unloading based on the taxonomy (e.g. category)', 'wp-asset-clean-up-pro').'</li>';
            }

            if ($importedKey === 'users_metas') {
	            $importedMessage .= '<li>' . esc_html__( 'Users\' Rules including unloading on all archive pages belonging to a specific user', 'wp-asset-clean-up-pro' ) . '</li>';
            }

            if ($importedKey === 'extras_exceptions') {
	            $importedMessage .= '<li>'.esc_html__('Load exceptions for all pages belonging to some or all of the following page types: archive, author, search, 404 pages', 'wp-asset-clean-up-pro').'</li>';
            }
        ...
        <div class="updated notice wpacu-notice wpacu-imported-notice is-dismissible">
            ...
            <p>Если вы используете плагин кэширования (например, WP Rocket, WP Fastest Cache, W3 Total Cache и т. д.), рекомендуется очистить его кеш, если веб-сайт работает так, как вы ожидаете после этого импорта, чтобы изменения вступали в силу для каждого посетителя.</p>
        </div>
        <?php
