...
HTML;
    ...
    public function frontendUpdate()
    {
        ...
        // [wpacu_pro]
        // The following is for the Pro version only
        ...
        UpdatePro::updateAssetsPositions();
        UpdatePro::updateHandleMediaQueriesLoad();
	    // [/wpacu_pro]
        ...
        UpdatePro::storeHardcodedAssetsInfo();
	    // [/wpacu_pro]
    }
...
SQL;
    ...
    public function savePost($postId, $post = '')
    {
	    ...
	    // This is triggered only if the "Asset CleanUp Pro" meta box was loaded with the list of assets (either in edit post/page or in "CSS & JS Manager" -> "Manage CSS/JS")
	    ...
	    // [wpacu_pro]
	    do_action('wpacu_pro_asset_info_update', 'post', $postId);
        ...
        UpdatePro::updateAssetsPositions();
        UpdatePro::updateHandleMediaQueriesLoad();
        UpdatePro::updateTaxonomyValuesAssocToPostType($post->post_type);
        UpdatePro::updateTaxonomyValuesAssocToPostTypeLoadExceptions($post->post_type);
		// [/wpacu_pro]
    ...
    public function updateFrontPage($wpacuNoLoadAssets)
    {
	    ...
	    // [wpacu_pro]
        // The following is for the Pro version only
        do_action('wpacu_pro_asset_info_update', 'homepage', 0);
	...
	public function saveLoadExceptions($type = 'post', $postId = '')
    {
        ...
        if (! empty($loadExceptions['styles']) || ! empty($loadExceptions['scripts'])) {
            ...
            /* [wpacu_pro] */
            elseif ($type === 'for_pro') {
	            // Update any load exceptions for pages like: taxonomy, 404, search, date etc.
	 ...
	 * @return bool
	 */
	public function removeBulkUnloads($value = '', $bulkType = 'post_type')
    {
