				...
				foreach ($linkTags as $tagObject) {
					...
					if (isset($linkAttributes['rel'], $linkAttributes['href']) && $linkAttributes['href']) {
						...
						if ($linkAttributes['rel'] === 'preload') {
							// [wpacu_pro]
							// Update: v1.1.5.0+ - Multiple Async preloaded CSS files are combined
							// The combined LINK will be kept preloaded with async
							if (isset($linkAttributes['data-wpacu-preload-it-async'])) {
								$mediaValue = 'wpacu_preload_async_' . $mediaValue;
							}
							// [/wpacu_pro]
							elseif (isset($linkAttributes['data-wpacu-preload-css-basic'])) {
								$mediaValue = 'wpacu_preload_basic_' . $mediaValue;
							} else {
								continue;
							}
						...
						// [wpacu_pro]
						if (isset($linkAttributes['id']) && (strpos($linkAttributes['id'], 'wpacu-moved-from-head-to-body-') !== false) && Main::instance()->settings['defer_css_loaded_body'] === 'moved') {
							$mediaValue = 'wpacu-moved-from-head-to-body-'.$mediaValue; // it will be renamed, needed for grouping
						}
						// [/wpacu_pro]
		...
		if ( ! empty($storageJsonContents) ) {
			foreach ($storageJsonContents as $docLocationTag => $mediaValues) {
				...
				foreach ($mediaValues as $mediaValue => $storageJsonContentLocation) {
					...
					// [wpacu_pro]
					if (strpos($mediaValue, 'wpacu_preload_async_') === 0) {
						// Put the right "media" value after cleaning the reference
						$mediaValueClean = str_replace('wpacu_preload_async_', '', $mediaValue);

						// Preload with Async CSS
						$finalCssTag = <<<HTML
<link rel='preload' as='style' onload="this.onload=null;this.rel='stylesheet'" data-wpacu-preload-it-async='1' id='wpacu-combined-css-{$docLocationTag}-{$groupLocation}-preload-it-async' href='{$finalTagUrl}' type='text/css' media='{$mediaValueClean}' />
HTML;
						$finalCssTagAttrs['rel']   = 'preload';
						$finalCssTagAttrs['media'] = $mediaValueClean;
					}
					// [/wpacu_pro]
					elseif (strpos($mediaValue, 'wpacu_preload_basic_') === 0) {
						// Put the right "media" value after cleaning the reference
