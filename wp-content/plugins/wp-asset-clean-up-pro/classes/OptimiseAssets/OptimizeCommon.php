...
class OptimizeCommon
{
		...
		/* [wpacu_timing] */ Misc::scriptExecTimer( 'fetch_strip_hardcoded_assets' ); /* [/wpacu_timing] */
			...
			/* [wpacu_timing] */ Misc::scriptExecTimer( 'fetch_all_hardcoded_assets' ); /* [/wpacu_timing] */
				...
				if ($userNotManagingInFrontEndView && ! empty($hardcodedMarkedForUnloadList)) {
					// There are hardcoded assets marked for unload
					$anyHardCodedAssetsList = HardcodedAssets::getAll( $htmlSource, false, true );
				} elseif (HardcodedAssets::useBufferingForEditFrontEndView()) {
					// For the admin only (if the CSS/JS list is managed in the front-end view)
					$anyHardCodedAssetsList = HardcodedAssets::getAll( $htmlSource, false, false );
	...
	public static function applyDebugTiming($htmlSource)
	{
		$timingKeys = array(
			...
			'local_fonts_display_style_inline',
			// [/wpacu_pro]
			...
			// [wpacu_pro]
			'fetch_marked_for_unload_hardcoded_assets',
			// [/wpacu_pro]
