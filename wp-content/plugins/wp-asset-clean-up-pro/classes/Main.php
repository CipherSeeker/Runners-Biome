...
SQL;
	...
	public function filterStyles()
	{
		...
		foreach ($list as $handle) {
			...
			if (strpos($handle, 'wpacu_hardcoded_link_') === 0) {
				...
				$this->allUnloadedAssets['styles'][] = $handle; // for "wpacu_no_load" on hardcoded list
    ...
    public function printScriptsStyles()
    {
        ...
        */
        ...
        if ($isFrontEndEditView) {
	        ...
	        /* [wpacu_pro] */ $list = apply_filters('wpacu_filter_list_on_dashboard_ajax_call', $list); /* [/wpacu_pro] */
    ...
    public function ajaxGetJsonListCallback()
    {
	    if ( ! isset($_POST['wpacu_nonce']) ) {
		    echo 'Ошибка: одноразовый номер безопасности не был отправлен для проверки. Расположение: '.__METHOD__;
		    return;
	    }

	    if ( ! wp_verify_nonce($_POST['wpacu_nonce'], 'wpacu_ajax_get_loaded_assets_nonce') ) {
		    echo 'Ошибка: проверка безопасности одноразового номера не удалась. Расположение: '.__METHOD__;
		    return;
	    }
            ...
            if ( ! ($wpacuListE || $wpacuListH) ) {
	            ...
	            if ( is_wp_error($wpRemotePost) ) {
		            ...
		            if ( trim( $wpRemotePost['body'] ) === '' ) {
			            $wpRemotePost['body'] = '<strong>Ошибка (пустая страница):</strong> похоже, что целевая страница загружается, но на ней нет содержимого. Страница кажется пустой. Пожалуйста, загрузите его в режиме инкогнито (когда вы не вошли в систему) через браузер.';
	...
	public function ajaxLoadRestrictedPageAreaCallback()
    {
	    if ( ! isset( $_POST['wpacu_nonce'] ) || ! wp_verify_nonce( $_POST['wpacu_nonce'], 'wpacu_ajax_load_page_restricted_area_nonce' ) ) {
		    echo 'Ошибка: одноразовый номер безопасности недействителен.';
		    exit();
	    }
	...
	public function ajaxCheckExternalUrlsForStatusCode()
    {
	    ...
	    if (! isset($_POST['action'], $_POST['wpacu_check_urls'])) {
		    echo 'Ошибка: параметры публикации неправильные.';
		    exit();
	    }
	...
	public function ajaxFetchActivePluginsIconsFromWordPressOrg()
	{
		...
		if ( ! wp_verify_nonce($_POST['wpacu_nonce'], 'wpacu_fetch_active_plugins_icons') ) {
			echo 'Ошибка: проверка безопасности не удалась. Расположение: '.__METHOD__;
			return;
		}
	...
	public function getAssetFileSize($obj, $format = 'for_print')
	{
		if (isset($obj->src) && $obj->src) {
			...
			if ($isExternalFile || $isLoadedOnTheFly) {
				return '<a class="wpacu-external-file-size" data-src="' . $src . '" href="#">рџ”— Get File Size</a>'.
	...
	public function ajaxGetExternalFileSize()
	{
		...
		// Check privileges
		if (! Menu::userCanManageAssets()) {
			echo 'Ошибка: Недостаточно прав для выполнения этого действия.';
			exit();
		}
		...
		if (stripos($remoteFile, '//fonts.googleapis.com/') !== false) {
			...
			// FontAwesome CDN
			echo ' + размеры загружаемых файлов шрифтов "FontAwesome" (см. "url" из @font-face в исходном файле)';
		}
