...
class BulkChanges
{
    ...
    public function pageBulkUnloads()
    {
	    ...
	    if (Misc::getVar('get', 'wpacu_bulk_menu_tab') === 'regex_unloads') {
			...
			*/
	        $this->data['values'] = \WpAssetCleanUpPro\MainPro::getRegExRules('unloads');
			...
			*/
	        $this->data['values'] = \WpAssetCleanUpPro\MainPro::getRegExRules('load_exceptions');
        } else {
            /*
	...
	public function noticeArchivePostTypesRemoved()
	{
        ...
        <div class="updated notice wpacu-notice is-dismissible">
            <p><span class="dashicons dashicons-yes"></span>
				<?php
				echo sprintf(
					__('The unload rule for the chosen styles/scripts were removed from the archive page of <strong><u>%s</u></strong> custom post type and they will now load again, unless you have other rules that would prevent them from loading.', 'wp-asset-clean-up-pro'),
	...
	public function noticeTaxonomiesRemoved()
	{
		?>
        <div class="updated notice wpacu-notice is-dismissible">
            <p><span class="dashicons dashicons-yes"></span> Выбранные стили/скрипты были удалены из списка выгрузки для<strong><u><?php echo esc_html($this->wpacuTaxonomy); ?></u></strong>
                тип таксономии, и теперь они будут загружаться снова, если только у вас нет других правил, препятствующих их загрузке.</p>
        </div>
		<?php
	}
	...
	public function noticeAuthorsRemoved()
	{
		?>
        <div class="updated notice wpacu-notice is-dismissible">
            <p><span class="dashicons dashicons-yes"></span> Выбранные стили/скрипты были удалены из списка выгрузки для всех страниц <strong>Автор</strong>, и теперь они будут загружаться снова, если только у вас нет других правил, запрещающих их загрузку.</p>
        </div>
		<?php
	}
	...
	public function noticeDatesRemoved()
	{
		?>
        <div class="updated notice wpacu-notice is-dismissible">
            <p><span class="dashicons dashicons-yes"></span> Выбранные стили/скрипты были удалены из списка выгрузки для всех страниц <strong>Даты</strong> (любая дата), и теперь они будут загружаться снова, если только у вас нет других правил, запрещающих их загрузку.</p>
        </div>
		<?php
	}
	...
	public function notice404Removed()
	{
		?>
        <div class="updated notice wpacu-notice is-dismissible">
            <p><span class="dashicons dashicons-yes"></span> Выбранные стили/скрипты были удалены из списка выгрузки для всех страниц <strong>404 Not Found</strong> (любой URL), и теперь они будут загружаться снова, если только у вас нет других правил, запрещающих их загрузку.</ р>
        </div>
		<?php
	}
	// [/wpacu_pro]
