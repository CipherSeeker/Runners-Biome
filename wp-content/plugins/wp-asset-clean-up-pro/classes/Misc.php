...
class Misc
{
	...
	public static function endsWith($string, $endsWithString)
	{
		...
		if (function_exists('wpacuEndsWith')) {
			return wpacuEndsWith($string, $endsWithString);
		}
		// [/wpacu_pro]
	 ...
	 * @return mixed
	 */
	public static function getPageUrl($postId, $type = '', $typeId = '')
    {
    	// [wpacu_pro]
...
SQL;
	...
	public static function handleHasAtLeastOneRule($data, $assetTypeKey)
	{
		...
		// [wpacu_pro]
		// Is the asset loading from a new location?
		if (isset($data['row']['obj']->position) && $data['row']['obj']->position) {
			$assetPositionNew     = isset( $data['row']['obj']->position_new ) ? $data['row']['obj']->position_new : $data['row']['obj']->position;
			$assetLocationChanged = ( $assetPositionNew !== $data['row']['obj']->position );
		...
		// Any async/defer attributes applied on page or site-wide?
		if ($assetTypeKey === 'scripts') {
			$isAsyncOnPage = isset($data['scripts_attributes']['on_this_page']['async']) && in_array($data['row']['obj']->handle,  $data['scripts_attributes']['on_this_page']['async']);
			$isDeferOnPage = isset($data['scripts_attributes']['on_this_page']['defer']) && in_array($data['row']['obj']->handle,  $data['scripts_attributes']['on_this_page']['defer']);
			$isAsyncGlobal = isset($data['scripts_attributes']['everywhere']['async'])   && (in_array($data['row']['obj']->handle, $data['scripts_attributes']['everywhere']['async']));
			$isDeferGlobal = isset($data['scripts_attributes']['everywhere']['defer'])   && (in_array($data['row']['obj']->handle, $data['scripts_attributes']['everywhere']['defer']));

			if ($isAsyncOnPage || $isDeferOnPage || $isAsyncGlobal || $isDeferGlobal) {
				return true;
			}
		}
		// [/wpacu_pro]
