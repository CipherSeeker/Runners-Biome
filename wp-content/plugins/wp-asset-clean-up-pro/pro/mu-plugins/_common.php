...
add_filter( 'pre_update_option_rewrite_rules', function ( $newValue ) {
	...
	if ( ! isset($GLOBALS['wpacu_rewrite_rules_original_serialised']) ) {
		...
		$GLOBALS['wpacu_rewrite_rules_original_serialised'] = $originalRewriteRulesSerialised;
	}

	if ( isset($GLOBALS['wpacu_rewrite_rules_original_serialised']) &&
	     is_serialized($GLOBALS['wpacu_rewrite_rules_original_serialised']) &&
	     $newValue ) {
		$newValueSerialised = maybe_serialize($newValue);

		if ( $newValueSerialised !== $GLOBALS['wpacu_rewrite_rules_original_serialised'] &&
		     strlen($GLOBALS['wpacu_rewrite_rules_original_serialised']) > strlen($newValueSerialised) )
		{
			// Restore it the way it was as it would be later serialised and updated
			$newValue = maybe_unserialize($GLOBALS['wpacu_rewrite_rules_original_serialised']);
		}
